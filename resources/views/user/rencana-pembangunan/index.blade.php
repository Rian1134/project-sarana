    ...
    <div class="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4 sm:mb-6">
            ...
            <x-button href="{{ route('user.rencana-pembangunan.create') }}" variant="primary"
                ...
                <i class="bi bi-plus-lg"></i> Buat Rencana
            </x-button>
        </div>
                    ...
                    <div class="flex items-center justify-between pt-2 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <div class="flex gap-1">
                            @if (!empty($item->lampiran))
                                <x-button href="{{ $item->lampiran }}" target="_blank" rel="noopener" variant="secondary"
                                    size="xs">
                        ...
                        @forelse ($pengajuans as $item)
                            ...
                            <x-table.row class="align-middle">
                                <x-table.cell class="font-medium px-3 py-2.5">
                                    <ul class="space-y-0.5">
                                        @foreach ($kategoriKeys as $kunci)
                                            <li>{{ \App\Http\Controllers\User\RencanaPembangunanController::categoryLabel($kunci) }}
                                            </li>
                                        @endforeach
                                        @if (count($tambahanKeys))
                                            <li>Perubahan Lainnya</li>
                                ...
                                <x-table.cell class="px-3 py-2.5">
                                    <ul class="space-y-1 text-sm">
                                        ...
                                        @if (count($tambahanKeys))
                                            <li>
                                                @if (count($kategoriKeys) > 0)
                                                    <div class="text-xs font-semibold text-gray-400 uppercase">Perubahan Lainnya
                                                    </div>
                                                @endif
                                                <ul class="space-y-0.5 pl-2">
                                                    @foreach ($tambahanKeys as $namaField)
                                    ...
                                    <div class="flex justify-center gap-1">
                                        ...
                                        @if ($item->status === 'pending')
                                            ...
                                            <form action="{{ route('user.rencana-pembangunan.destroy', $item) }}"
                                                method="POST" onsubmit="return confirm('Hapus rencana ini?')">
                                                @csrf
                                                @method('DELETE')
                                                <x-button type="submit" variant="danger" size="xs">
                                                    <i class="bi bi-trash-fill"></i>
                            ...
                            @empty
                                <x-table.empty colspan="5" message="Belum ada rencana pembangunan." />
                            @endforelse
                        </x-table>
                    </div>
                </div>
