...
<div class="max-w-5xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
    <div class="flex flex-col gap-4 sm:gap-6">
        ...
        @php
            $kategoriKeys = is_array($pengajuan->pengajuan) ? $pengajuan->pengajuan : array_filter([$pengajuan->pengajuan]);
        ...
        <div class="flex flex-col sm:flex-row sm:justify-end gap-2">
            <x-button href="{{ route('user.rencana-pembangunan.index') }}" variant="light" class="w-full sm:w-auto justify-center">
                Kembali
            </x-button>
            @if ($pengajuan->status === 'pending')
                <x-button href="{{ route('user.rencana-pembangunan.edit', $pengajuan) }}" variant="warning" class="w-full sm:w-auto justify-center">
                    Edit
                </x-button>
            @endif
        </div>
